...
for task in range(config["num_tasks"]):
    ...
    # Reset task (no reset for weights)
    ...
    torch.save(agent.q_network.state_dict(), f"logs/dqn_task_{task + 1}_never_reset.pt")

# Log and plot metrics
log_metrics(all_rewards, "logs/rewards_never_reset.json")
